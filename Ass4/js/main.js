document.getElementById("loginid").onkeyup = function() {
	...
	if(data != "") {
		if(/\d/.test(data)) {
			document.getElementById("label_loginid").innerHTML = "Must not contain digits";
			document.getElementById("loginid").classList.remove("input-field");
		}
		else
		{
			if(data.length < 8) {
				document.getElementById("label_loginid").innerHTML = "Must be of minimum length 8";
				document.getElementById("loginid").classList.remove("input-field");
			}
			else {
...
document.getElementById("loginbtn").onclick = function() {
	...
	if(login_id == "")
		document.getElementById("label_loginid").innerHTML = "Username is required";
	if(password == "")
